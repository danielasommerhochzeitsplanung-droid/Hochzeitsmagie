...
export default function VendorsModule() {
  ...
  const syncBudgetFromVendor = async (vendorId: string, vendorData: Omit<Vendor, 'id' | 'created_at'>) => {
    ...
    const budgetData = {
      category: vendorData.category,
      item_name: vendorData.company_name,
      planned_amount: vendorData.price_estimate || 0,
      actual_amount: vendorData.final_price || 0,
      payment_status: getPaymentStatus(vendorData.payment_status),
      vendor_id: vendorId,
      is_auto_synced: true,
      notes: `Automatisch synchronisiert mit ${vendorData.company_name}`
    };
  ...
  const getPaymentStatus = (vendorPaymentStatus: string): 'open' | 'paid' | 'partial' => {
    if (vendorPaymentStatus === 'Vollständig bezahlt') return 'paid';
    if (vendorPaymentStatus === 'Anzahlung geleistet' || vendorPaymentStatus === 'Restbetrag ausstehend') return 'partial';
    return 'open';
  };

  const saveEventDayContacts = async (vendorId: string, contacts: VendorEventDayContact[]) => {
    await supabase
      .from('vendor_event_day_contacts')
      .delete()
      .eq('vendor_id', vendorId);
  ...
  const saveEventAssignments = async (vendorId: string, eventIds: string[]) => {
    await supabase
    ...
    if (eventIds.length > 0) {
      const assignmentsToInsert = eventIds.map(eventId => ({
        vendor_id: vendorId,
        event_id: eventId
      }));
